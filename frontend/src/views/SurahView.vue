...
async function loadSurah(id) {
  ...
  await store.fetchVersesByChapter(id, 1)
}

async function loadMore() {
  if (!store.pagination || !store.pagination.next_page || loadingMore.value) return
  loadingMore.value = true
  await store.fetchVersesByChapter(chapterId.value, store.pagination.next_page)
...
<template>
  <div class="surah-view">
    ...
    <!-- Bismillah -->
    <div v-if="showBismillah" class="bismillah" translate="no">
